...
public class ProfessorRegisterRequestFilter : IFilter<ProfessorRegisterRequest>
{
      public IQueryable<ProfessorRegisterRequest> Filter(IQueryable<ProfessorRegisterRequest> filterQuery, Dictionary<string, string> filterBy)
    {
        foreach (var (propertyName, propertyValue) in filterBy)
        {
            var propertyInfo = typeof(ProfessorRegisterRequest).GetProperties().FirstOrDefault(property => string.Equals(property.Name, propertyName, StringComparison.InvariantCultureIgnoreCase));
